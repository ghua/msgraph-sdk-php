...
class PrinterCapabilities implements AdditionalDataHolder, Parsable 
{
    ...
    */
    public function getFieldDeserializers(): array {
        $currentObject = $this;
        return  [
            'bottomMargins' => function (ParseNode $n) use ($currentObject) { $currentObject->setBottomMargins($n->getCollectionOfPrimitiveValues()); },
            'collation' => function (ParseNode $n) use ($currentObject) { $currentObject->setCollation($n->getBooleanValue()); },
            'colorModes' => function (ParseNode $n) use ($currentObject) { $currentObject->setColorModes($n->getCollectionOfEnumValues(PrintColorMode::class)); },
            'contentTypes' => function (ParseNode $n) use ($currentObject) { $currentObject->setContentTypes($n->getCollectionOfPrimitiveValues()); },
            'copiesPerJob' => function (ParseNode $n) use ($currentObject) { $currentObject->setCopiesPerJob($n->getObjectValue(IntegerRange::class)); },
            'dpis' => function (ParseNode $n) use ($currentObject) { $currentObject->setDpis($n->getCollectionOfPrimitiveValues()); },
            'duplexModes' => function (ParseNode $n) use ($currentObject) { $currentObject->setDuplexModes($n->getCollectionOfEnumValues(PrintDuplexMode::class)); },
            'feedOrientations' => function (ParseNode $n) use ($currentObject) { $currentObject->setFeedOrientations($n->getCollectionOfEnumValues(PrinterFeedOrientation::class)); },
            ...
            'inputBins' => function (ParseNode $n) use ($currentObject) { $currentObject->setInputBins($n->getCollectionOfPrimitiveValues()); },
            'isColorPrintingSupported' => function (ParseNode $n) use ($currentObject) { $currentObject->setIsColorPrintingSupported($n->getBooleanValue()); },
            'isPageRangeSupported' => function (ParseNode $n) use ($currentObject) { $currentObject->setIsPageRangeSupported($n->getBooleanValue()); },
            'leftMargins' => function (ParseNode $n) use ($currentObject) { $currentObject->setLeftMargins($n->getCollectionOfPrimitiveValues()); },
            'mediaColors' => function (ParseNode $n) use ($currentObject) { $currentObject->setMediaColors($n->getCollectionOfPrimitiveValues()); },
            'mediaSizes' => function (ParseNode $n) use ($currentObject) { $currentObject->setMediaSizes($n->getCollectionOfPrimitiveValues()); },
            'mediaTypes' => function (ParseNode $n) use ($currentObject) { $currentObject->setMediaTypes($n->getCollectionOfPrimitiveValues()); },
            'multipageLayouts' => function (ParseNode $n) use ($currentObject) { $currentObject->setMultipageLayouts($n->getCollectionOfEnumValues(PrintMultipageLayout::class)); },
            'orientations' => function (ParseNode $n) use ($currentObject) { $currentObject->setOrientations($n->getCollectionOfEnumValues(PrintOrientation::class)); },
            'outputBins' => function (ParseNode $n) use ($currentObject) { $currentObject->setOutputBins($n->getCollectionOfPrimitiveValues()); },
            'pagesPerSheet' => function (ParseNode $n) use ($currentObject) { $currentObject->setPagesPerSheet($n->getCollectionOfPrimitiveValues()); },
            'qualities' => function (ParseNode $n) use ($currentObject) { $currentObject->setQualities($n->getCollectionOfEnumValues(PrintQuality::class)); },
            'rightMargins' => function (ParseNode $n) use ($currentObject) { $currentObject->setRightMargins($n->getCollectionOfPrimitiveValues()); },
            'scalings' => function (ParseNode $n) use ($currentObject) { $currentObject->setScalings($n->getCollectionOfEnumValues(PrintScaling::class)); },
            'supportsFitPdfToPage' => function (ParseNode $n) use ($currentObject) { $currentObject->setSupportsFitPdfToPage($n->getBooleanValue()); },
            'topMargins' => function (ParseNode $n) use ($currentObject) { $currentObject->setTopMargins($n->getCollectionOfPrimitiveValues()); },
        ];
    }
