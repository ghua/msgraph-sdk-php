...
{
    /** @var UnifiedRoleScheduleRequestActions|null $action Represents the type of the operation on the role assignment. The possible values are: AdminAssign: For administrators to assign roles to users or groups.AdminRemove: For administrators to remove users or groups from roles. AdminUpdate: For administrators to change existing role assignments.AdminExtend: For administrators to extend expiring assignments.AdminRenew: For administrators to renew expired assignments.SelfActivate: For users to activate their assignments.SelfDeactivate: For users to deactivate their active assignments.SelfExtend: For users to request to extend their expiring assignments.SelfRenew: For users to request to renew their expired assignments. */
    ...
    */
    public function getFieldDeserializers(): array {
        ...
        return array_merge(parent::getFieldDeserializers(), [
            'action' => function (ParseNode $n) use ($currentObject) { $currentObject->setAction($n->getEnumValue(UnifiedRoleScheduleRequestActions::class)); },
            'activatedUsing' => function (ParseNode $n) use ($currentObject) { $currentObject->setActivatedUsing($n->getObjectValue(UnifiedRoleEligibilitySchedule::class)); },
            ...
            'isValidationOnly' => function (ParseNode $n) use ($currentObject) { $currentObject->setIsValidationOnly($n->getBooleanValue()); },
            ...
            'scheduleInfo' => function (ParseNode $n) use ($currentObject) { $currentObject->setScheduleInfo($n->getObjectValue(RequestSchedule::class)); },
            'targetSchedule' => function (ParseNode $n) use ($currentObject) { $currentObject->setTargetSchedule($n->getObjectValue(UnifiedRoleAssignmentSchedule::class)); },
            'targetScheduleId' => function (ParseNode $n) use ($currentObject) { $currentObject->setTargetScheduleId($n->getStringValue()); },
            'ticketInfo' => function (ParseNode $n) use ($currentObject) { $currentObject->setTicketInfo($n->getObjectValue(TicketInfo::class)); },
        ]);
    }
