    ...
    */
    public function getFieldDeserializers(): array {
        ...
        return array_merge(parent::getFieldDeserializers(), [
            ...
            'customers' => function (ParseNode $n) use ($currentObject) { $currentObject->setCustomers($n->getCollectionOfObjectValues(BookingCustomerInformationBase::class)); },
            'customerTimeZone' => function (ParseNode $n) use ($currentObject) { $currentObject->setCustomerTimeZone($n->getStringValue()); },
            'duration' => function (ParseNode $n) use ($currentObject) { $currentObject->setDuration($n->getDateIntervalValue()); },
            'endDateTime' => function (ParseNode $n) use ($currentObject) { $currentObject->setEndDateTime($n->getObjectValue(DateTimeTimeZone::class)); },
            'filledAttendeesCount' => function (ParseNode $n) use ($currentObject) { $currentObject->setFilledAttendeesCount($n->getIntegerValue()); },
            ...
            'optOutOfCustomerEmail' => function (ParseNode $n) use ($currentObject) { $currentObject->setOptOutOfCustomerEmail($n->getBooleanValue()); },
            ...
            'price' => function (ParseNode $n) use ($currentObject) { $currentObject->setPrice($n->getFloatValue()); },
            'priceType' => function (ParseNode $n) use ($currentObject) { $currentObject->setPriceType($n->getEnumValue(BookingPriceType::class)); },
            'reminders' => function (ParseNode $n) use ($currentObject) { $currentObject->setReminders($n->getCollectionOfObjectValues(BookingReminder::class)); },
            'selfServiceAppointmentId' => function (ParseNode $n) use ($currentObject) { $currentObject->setSelfServiceAppointmentId($n->getStringValue()); },
            'serviceId' => function (ParseNode $n) use ($currentObject) { $currentObject->setServiceId($n->getStringValue()); },
            'serviceLocation' => function (ParseNode $n) use ($currentObject) { $currentObject->setServiceLocation($n->getObjectValue(Location::class)); },
            'serviceName' => function (ParseNode $n) use ($currentObject) { $currentObject->setServiceName($n->getStringValue()); },
            'serviceNotes' => function (ParseNode $n) use ($currentObject) { $currentObject->setServiceNotes($n->getStringValue()); },
