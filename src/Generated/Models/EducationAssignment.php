    ...
    */
    public function getFieldDeserializers(): array {
        ...
        return array_merge(parent::getFieldDeserializers(), [
            ...
            'allowLateSubmissions' => function (ParseNode $n) use ($currentObject) { $currentObject->setAllowLateSubmissions($n->getBooleanValue()); },
            'allowStudentsToAddResourcesToSubmission' => function (ParseNode $n) use ($currentObject) { $currentObject->setAllowStudentsToAddResourcesToSubmission($n->getBooleanValue()); },
            'assignDateTime' => function (ParseNode $n) use ($currentObject) { $currentObject->setAssignDateTime($n->getDateTimeValue()); },
            'assignedDateTime' => function (ParseNode $n) use ($currentObject) { $currentObject->setAssignedDateTime($n->getDateTimeValue()); },
            'assignTo' => function (ParseNode $n) use ($currentObject) { $currentObject->setAssignTo($n->getObjectValue(EducationAssignmentRecipient::class)); },
            'categories' => function (ParseNode $n) use ($currentObject) { $currentObject->setCategories($n->getCollectionOfObjectValues(EducationCategory::class)); },
            'classId' => function (ParseNode $n) use ($currentObject) { $currentObject->setClassId($n->getStringValue()); },
            'closeDateTime' => function (ParseNode $n) use ($currentObject) { $currentObject->setCloseDateTime($n->getDateTimeValue()); },
            ...
            'instructions' => function (ParseNode $n) use ($currentObject) { $currentObject->setInstructions($n->getObjectValue(EducationItemBody::class)); },
            ...
            'resources' => function (ParseNode $n) use ($currentObject) { $currentObject->setResources($n->getCollectionOfObjectValues(EducationAssignmentResource::class)); },
            'resourcesFolderUrl' => function (ParseNode $n) use ($currentObject) { $currentObject->setResourcesFolderUrl($n->getStringValue()); },
            'rubric' => function (ParseNode $n) use ($currentObject) { $currentObject->setRubric($n->getObjectValue(EducationRubric::class)); },
            'status' => function (ParseNode $n) use ($currentObject) { $currentObject->setStatus($n->getEnumValue(EducationAssignmentStatus::class)); },
            'submissions' => function (ParseNode $n) use ($currentObject) { $currentObject->setSubmissions($n->getCollectionOfObjectValues(EducationSubmission::class)); },
