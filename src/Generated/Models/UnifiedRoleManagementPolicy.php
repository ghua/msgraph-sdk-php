    ...
    */
    public function getFieldDeserializers(): array {
        ...
        return array_merge(parent::getFieldDeserializers(), [
            ...
            'effectiveRules' => function (ParseNode $n) use ($currentObject) { $currentObject->setEffectiveRules($n->getCollectionOfObjectValues(UnifiedRoleManagementPolicyRule::class)); },
            'isOrganizationDefault' => function (ParseNode $n) use ($currentObject) { $currentObject->setIsOrganizationDefault($n->getBooleanValue()); },
            'lastModifiedBy' => function (ParseNode $n) use ($currentObject) { $currentObject->setLastModifiedBy($n->getObjectValue(Identity::class)); },
            'lastModifiedDateTime' => function (ParseNode $n) use ($currentObject) { $currentObject->setLastModifiedDateTime($n->getDateTimeValue()); },
            'rules' => function (ParseNode $n) use ($currentObject) { $currentObject->setRules($n->getCollectionOfObjectValues(UnifiedRoleManagementPolicyRule::class)); },
            'scopeId' => function (ParseNode $n) use ($currentObject) { $currentObject->setScopeId($n->getStringValue()); },
            'scopeType' => function (ParseNode $n) use ($currentObject) { $currentObject->setScopeType($n->getStringValue()); },
        ]);
    }
