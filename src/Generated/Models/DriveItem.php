    ...
    */
    public function getFieldDeserializers(): array {
        ...
        return array_merge(parent::getFieldDeserializers(), [
            ...
            'audio' => function (ParseNode $n) use ($currentObject) { $currentObject->setAudio($n->getObjectValue(Audio::class)); },
            'bundle' => function (ParseNode $n) use ($currentObject) { $currentObject->setBundle($n->getObjectValue(Bundle::class)); },
            'children' => function (ParseNode $n) use ($currentObject) { $currentObject->setChildren($n->getCollectionOfObjectValues(DriveItem::class)); },
            'content' => function (ParseNode $n) use ($currentObject) { $currentObject->setContent($n->getBinaryContent()); },
            'cTag' => function (ParseNode $n) use ($currentObject) { $currentObject->setCTag($n->getStringValue()); },
            'deleted' => function (ParseNode $n) use ($currentObject) { $currentObject->setDeleted($n->getObjectValue(Deleted::class)); },
            ...
            'location' => function (ParseNode $n) use ($currentObject) { $currentObject->setLocation($n->getObjectValue(GeoCoordinates::class)); },
            'malware' => function (ParseNode $n) use ($currentObject) { $currentObject->setMalware($n->getObjectValue(Malware::class)); },
            'package' => function (ParseNode $n) use ($currentObject) { $currentObject->setPackage($n->getObjectValue(Package::class)); },
            'pendingOperations' => function (ParseNode $n) use ($currentObject) { $currentObject->setPendingOperations($n->getObjectValue(PendingOperations::class)); },
            ...
            'photo' => function (ParseNode $n) use ($currentObject) { $currentObject->setPhoto($n->getObjectValue(Photo::class)); },
            'publication' => function (ParseNode $n) use ($currentObject) { $currentObject->setPublication($n->getObjectValue(PublicationFacet::class)); },
            'remoteItem' => function (ParseNode $n) use ($currentObject) { $currentObject->setRemoteItem($n->getObjectValue(RemoteItem::class)); },
            'root' => function (ParseNode $n) use ($currentObject) { $currentObject->setRoot($n->getObjectValue(Root::class)); },
            'searchResult' => function (ParseNode $n) use ($currentObject) { $currentObject->setSearchResult($n->getObjectValue(SearchResult::class)); },
            ...
            'thumbnails' => function (ParseNode $n) use ($currentObject) { $currentObject->setThumbnails($n->getCollectionOfObjectValues(ThumbnailSet::class)); },
            'versions' => function (ParseNode $n) use ($currentObject) { $currentObject->setVersions($n->getCollectionOfObjectValues(DriveItemVersion::class)); },
            ...
            'workbook' => function (ParseNode $n) use ($currentObject) { $currentObject->setWorkbook($n->getObjectValue(Workbook::class)); },
        ]);
    }
