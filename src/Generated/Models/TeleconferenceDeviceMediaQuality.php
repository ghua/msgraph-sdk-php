...
class TeleconferenceDeviceMediaQuality implements AdditionalDataHolder, Parsable 
{
    ...
    */
    public function getFieldDeserializers(): array {
        $currentObject = $this;
        return  [
            'averageInboundJitter' => function (ParseNode $n) use ($currentObject) { $currentObject->setAverageInboundJitter($n->getDateIntervalValue()); },
            'averageInboundPacketLossRateInPercentage' => function (ParseNode $n) use ($currentObject) { $currentObject->setAverageInboundPacketLossRateInPercentage($n->getFloatValue()); },
            'averageInboundRoundTripDelay' => function (ParseNode $n) use ($currentObject) { $currentObject->setAverageInboundRoundTripDelay($n->getDateIntervalValue()); },
            'averageOutboundJitter' => function (ParseNode $n) use ($currentObject) { $currentObject->setAverageOutboundJitter($n->getDateIntervalValue()); },
            'averageOutboundPacketLossRateInPercentage' => function (ParseNode $n) use ($currentObject) { $currentObject->setAverageOutboundPacketLossRateInPercentage($n->getFloatValue()); },
            'averageOutboundRoundTripDelay' => function (ParseNode $n) use ($currentObject) { $currentObject->setAverageOutboundRoundTripDelay($n->getDateIntervalValue()); },
            'channelIndex' => function (ParseNode $n) use ($currentObject) { $currentObject->setChannelIndex($n->getIntegerValue()); },
            'inboundPackets' => function (ParseNode $n) use ($currentObject) { $currentObject->setInboundPackets($n->getIntegerValue()); },
            'localIPAddress' => function (ParseNode $n) use ($currentObject) { $currentObject->setLocalIPAddress($n->getStringValue()); },
            'localPort' => function (ParseNode $n) use ($currentObject) { $currentObject->setLocalPort($n->getIntegerValue()); },
            'maximumInboundJitter' => function (ParseNode $n) use ($currentObject) { $currentObject->setMaximumInboundJitter($n->getDateIntervalValue()); },
            'maximumInboundPacketLossRateInPercentage' => function (ParseNode $n) use ($currentObject) { $currentObject->setMaximumInboundPacketLossRateInPercentage($n->getFloatValue()); },
            'maximumInboundRoundTripDelay' => function (ParseNode $n) use ($currentObject) { $currentObject->setMaximumInboundRoundTripDelay($n->getDateIntervalValue()); },
            'maximumOutboundJitter' => function (ParseNode $n) use ($currentObject) { $currentObject->setMaximumOutboundJitter($n->getDateIntervalValue()); },
            'maximumOutboundPacketLossRateInPercentage' => function (ParseNode $n) use ($currentObject) { $currentObject->setMaximumOutboundPacketLossRateInPercentage($n->getFloatValue()); },
            'maximumOutboundRoundTripDelay' => function (ParseNode $n) use ($currentObject) { $currentObject->setMaximumOutboundRoundTripDelay($n->getDateIntervalValue()); },
            'mediaDuration' => function (ParseNode $n) use ($currentObject) { $currentObject->setMediaDuration($n->getDateIntervalValue()); },
            'networkLinkSpeedInBytes' => function (ParseNode $n) use ($currentObject) { $currentObject->setNetworkLinkSpeedInBytes($n->getIntegerValue()); },
            'outboundPackets' => function (ParseNode $n) use ($currentObject) { $currentObject->setOutboundPackets($n->getIntegerValue()); },
            'remoteIPAddress' => function (ParseNode $n) use ($currentObject) { $currentObject->setRemoteIPAddress($n->getStringValue()); },
            'remotePort' => function (ParseNode $n) use ($currentObject) { $currentObject->setRemotePort($n->getIntegerValue()); },
        ];
    }
