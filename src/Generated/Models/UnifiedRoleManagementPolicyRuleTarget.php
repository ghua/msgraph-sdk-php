...
class UnifiedRoleManagementPolicyRuleTarget implements AdditionalDataHolder, Parsable 
{
    ...
    /** @var array<UnifiedRoleManagementPolicyRuleTargetOperations>|null $operations The operations for policy rule target. Allowed values are: All, Activate, Deactivate, Assign, Update, Remove, Extend, Renew. */
    ...
    */
    public function getFieldDeserializers(): array {
        $currentObject = $this;
        return  [
            'caller' => function (ParseNode $n) use ($currentObject) { $currentObject->setCaller($n->getStringValue()); },
            'enforcedSettings' => function (ParseNode $n) use ($currentObject) { $currentObject->setEnforcedSettings($n->getCollectionOfPrimitiveValues()); },
            'inheritableSettings' => function (ParseNode $n) use ($currentObject) { $currentObject->setInheritableSettings($n->getCollectionOfPrimitiveValues()); },
            'level' => function (ParseNode $n) use ($currentObject) { $currentObject->setLevel($n->getStringValue()); },
            'operations' => function (ParseNode $n) use ($currentObject) { $currentObject->setOperations($n->getCollectionOfEnumValues(UnifiedRoleManagementPolicyRuleTargetOperations::class)); },
            'targetObjects' => function (ParseNode $n) use ($currentObject) { $currentObject->setTargetObjects($n->getCollectionOfObjectValues(DirectoryObject::class)); },
        ];
    }
